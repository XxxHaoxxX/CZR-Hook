...
struct Il2CppDictionary {
    ...
    Il2CppArray<Entry>* entries; // 正确顺序：Entry 已在上方定义
    int count;
    int version;
    int freeList;
    int freeCount;
    ...
    Entry* getEntries() {
        return entries ? entries->getPointer() : nullptr;
    }
...
template<typename K, typename V>
struct Il2CppDictionary {
    Il2CppClass* klass;        // 偏移 0x0 (8字节)
    void* monitor;             // 偏移 0x8 (8字节)
    Il2CppArray<int>* buckets; // 偏移 0x10 (8字节)
    Il2CppArray<Entry>* entries;// 偏移 0x18 (8字节)
    int count;                 // 偏移 0x20 (4字节)
    int version;               // 偏移 0x24 (4字节)
    int freeList;              // 偏移 0x28 (4字节)
    int freeCount;             // 偏移 0x2C (4字节)
    // 总大小: 0x30 (48字节)
    
    // Entry 结构的内存布局
    struct Entry {
        int hashCode;          // 偏移 0x0 (4字节)
        int next;              // 偏移 0x4 (4字节)
        K key;                 // 偏移 0x8 (大小取决于K)
        V value;               // 偏移 0x8 + sizeof(K) (大小取决于V)
    };
};
*/
